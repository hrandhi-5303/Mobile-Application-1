...
class FirestoreController {
  ...
  static Future<void> updatePhotoMemoCommentCount(
    ...
    print("Updated the comment count");
    // doc id auto-generated.
  }
  ...
  static reportComment(
     ...
     await FirebaseFirestore.instance
        .collection(Constant.photoCommentCollection).doc(commentId).update({
       'reportedBy':FieldValue.arrayUnion([FirebaseAuth.instance.currentUser!.uid])
    });
  }
  static removeFromReportedComment(
      {required String commentId}) async {
     await FirebaseFirestore.instance
        .collection(Constant.photoCommentCollection).doc(commentId).update({
       'reportedBy':FieldValue.arrayRemove([FirebaseAuth.instance.currentUser!.uid])
    });
  }
  ...
  static Future<List<PhotoMemo>> getPhotoMemoList({
    ...
    developer.log('memo 2');
    for (var doc in querySnapshot.docs) {
      if (doc.data() != null) {
        developer.log('memo 3');
        var document = doc.data() as Map<String, dynamic>;
        var p = PhotoMemo.fromFirestoreDoc(doc: document, docId: doc.id);
        if (p != null) result.add(p);
      }
    }
    developer.log('memo 4');
        //print("****\nResult: ${result.toList().toString()}\n***");
