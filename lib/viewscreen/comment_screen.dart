...
import 'package:lesson3/util/alerts.dart';
                      ...
                      child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            return Column(
                              children: [
                                ListTile(
                                  tileColor: controller.photoCommentList[index].reportedBy.contains( controller.state.widget.user.uid)?Colors.red.withOpacity(0.5):Colors.white,
                                  trailing: controller.state.widget.user.uid ==
                                      ...
                                      : IconButton(
                                          onPressed: (){
                                             if(controller.photoCommentList[index].reportedBy.contains( controller.state.widget.user.uid)){
                                               MyAlerts.showAlertDialog(context, "REMOVE", "Do you want to remove it from reported comments", "Remove", (){
                                                 Navigator.pop(context);
                                                 setState(() {
                                                   FirestoreController.removeFromReportedComment(commentId:controller.photoCommentList[index].commentID);
                                                   controller.photoCommentList[index].reportedBy.remove( controller.state.widget.user.uid);
                                                 });
                                               });
                                             }else{
                                               MyAlerts.showAlertDialog(context, "REPORT COMMENT", "Do you want to report this comment", "Report", (){
                                                 Navigator.pop(context);
                                                 setState(() {
                                                 FirestoreController.reportComment(commentId:controller.photoCommentList[index].commentID);
                                                 controller.photoCommentList[index].reportedBy.add( controller.state.widget.user.uid);
                                               });
                                               });
                                             }

                                          },
                                          icon: Icon(Icons.report_gmailerrorred_outlined,color: Colors.red,),
                                        ),
...
class _Controller {
  ...
  _Controller(this.state) {
   // photoComment = PhotoComment.clone(state.widget.);
    tempMemo = PhotoMemo.clone(state.widget.photoMemo);
    photoCommentList = state.widget.photoComment;
  }
  String? comment = '';
